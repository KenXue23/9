...
def display_result():
    ...
    wrong_count = 10 - correct_answers.count(True)
    score = int(correct_answers.count(True) / 10 * 100)
    result = f"Score: {score}%\nCorrect: {correct_answers.count(True)}\nWrong: {wrong_count}"
...
def display_question():
    ...
    opt_selected.set(0)
    correct_answer = correct_answers[random_question_index]
...
def check_ans():
    global current_question, correct_answers, question_number
    
    selected_option = opt_selected.get()
    if selected_option == correct_answers[current_question]:
        correct_answers[current_question] = True
...
def reveal_correct_answer():
    ...
    correct_option_index = ord(correct_option) - ord("A")  # Convert letter to index
...
displaybar = ImageTk.PhotoImage(Image.open('C:/Users/kx063/OneDrive/桌面/91906/assest/Rounded Rectangle 3.png').resize((500, 100)), Image.ANTIALIAS)
nextbutton = ImageTk.PhotoImage(Image.open('C:/Users/kx063/OneDrive/桌面/91906/assest/next.png').resize((200, 90)), Image.ANTIALIAS)
...
question_number_label = Label(root, text="", font=("Helvetica", 20))
